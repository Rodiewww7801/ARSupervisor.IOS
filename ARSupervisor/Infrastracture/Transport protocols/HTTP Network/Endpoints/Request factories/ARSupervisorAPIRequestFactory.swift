...
class BackendAPIRequestFactory {
    ...
    static func register(with dto: RegisterRequestDTO) -> RequestModel {
        ...
        var model = RequestModel(basePath: serverAPI, path: ARSupervisorAPIPath.register, httpMethod: .post)
    ...
    static func login(with dto: LoginRequestDTO) -> RequestModel {
        let encodedData = try? JSONEncoder().encode(dto)
        var model = RequestModel(basePath: serverAPI, path: ARSupervisorAPIPath.login, httpMethod: .post)
        model.body = encodedData
        return model
    }
    ...
    static func refresh(with dto: RefreshRequestDTO) -> RequestModel {
        let encodedData = try? JSONEncoder().encode(dto)
        var model = RequestModel(basePath: serverAPI, path: ARSupervisorAPIPath.refresh, httpMethod: .post)
        model.body = encodedData
        return model
    }
